...
class Parser:
  ...
  def _import_statement(self) -> ast.ImportStatement:
    """IMPORTAR \"modulo\"  o  IMPORTAR modulo"""
    line = self._advance().line  # consume IMPORTAR
    if self._check(TokenType.STRING):
      module = self._advance().value
    elif self._check(TokenType.IDENTIFIER):
      module = self._advance().lexeme
    else:
      raise ParserError(
          "¡Error de Ki Sintáctico! Se esperaba el nombre del módulo tras IMPORTAR (ej: IMPORTAR \"Biblioteca\")",
          line,
      )
    return ast.ImportStatement(module=module, line=line)
